...
class RayCasting:
  ...
  def __init__(self, m_x, m_y, m_theta, step_size, global_map):
    ...
    self.z_required = np.ndarray((m_x, m_y, 2 * m_theta), np.int32)
        ...
        if not (self.global_map[i][j] < 0):
   	  # iterate over theta
          for k in range(-self.m_th, self.m_th, 3):
            # calculate distance
            self.z_required[i][j][k] = self.calculateDistance(i, j, k)
    return
  ...
  def calculateDistance(self, x, y, th):
    ...
    return ma.sqrt(pow(dx,2) + pow(dy,2))
